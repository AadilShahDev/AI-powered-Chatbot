...
const OpenRouterDirect = () => {
  ...
  const handleSend = async () => {
    ...
    const newMessages = [...conversation, { sender: 'user', message: userInput }];
    ...
    try {
      const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
        method: 'POST',
        headers: {
          'Authorization': 'Bearer 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',
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({
          model: 'mistralai/mixtral-8x7b-instruct',
      ...
      setConversation(prev => [...prev, { sender: 'bot', message: botReply }]);
    } catch (err) {
      console.error(err);
      setConversation(prev => [...prev, { sender: 'bot', message: 'Failed to fetch AI response.' }]);
    }
    setLoading(false);
  };
